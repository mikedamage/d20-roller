...
Parser.prototype._nextToken = function _nextToken() {
  ...
  if (this._pos >= this._length) {
    ...
    ret.type   = 'number';
    res        = this._fetchNumber();
    this._pos += res.length - 1;
    ret.value  = res.value;
  } else if (operators.hasOwnProperty(ch)) {
    ret.type       = 'operator';
    ret.precedence = operators[ch].precedence;
  } else {
    var err    = new Error('Invalid character ' + ch);
    err.column = this._pos;
    throw err;
  }
};

Parser.prototype._fetchNumber = function _fetchNumber() {
